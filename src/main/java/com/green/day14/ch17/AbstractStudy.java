...
public abstract class AbstractStudy {
    ...
    클래스 키워드 앞에 abstract 키워드를 붙이면 추상클래스가 된다.
    추상클래스가 되면 객체화 불가능 >> 부모역할만 한다. (단독으로는 의미가 없다)
    그래서 추상클래스를 상속받는 클래스가 있기 마련이다.

    abstract를 붙이는 것은 선택사항이나 만약에 추상 메소드를 가지게 된다면
    선택사항이 아니라 필수사항이 된다.

    클래스간의 공통적인 부분을 부모에 작성을 해서 상속받도록 하기 위한 것.
    그리고 추상 메소드를 구현해야하는 강재성!!이 존재한다.
    추상 메소드 구현은: implements, 구현한다라고 표현
     */

    //추상 메소드 정의 (선언부가 있고 구현부가 없는 메소드를 추상 메소드라 한다.
